module Acoustic2D_CUDA
    ...
    Compute `nt` timesteps of the acoustic 2D wave equation using ParallelStencil on xPUs (with `CUDA`) with CPML boundary conditions on a model with size `lx`x`ly` meters,
